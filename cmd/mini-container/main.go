...
// Command mini-container is a from-scratch container runtime built on raw Linux
// kernel primitives: namespaces, cgroups v2 and chroot.
//
// It only builds and runs on Linux. Windows and macOS users should use
// mini-container-launcher, which provisions this binary inside a Linux
// environment (WSL2, Lima or Docker) and runs it there.
package main
...
// rootfsPath is the directory the container is jailed into. It is relative to
// the working directory by default so the Makefile's "assets/rootfs" layout
// keeps working; the launcher overrides it via MINI_CONTAINER_ROOTFS.
func rootfsPath() string {
	if p := os.Getenv("MINI_CONTAINER_ROOTFS"); p != "" {
		return p
	}
...
func parent() {
	if len(os.Args) < 3 {
		fmt.Fprintln(os.Stderr, "usage: mini-container run <command> [args...]")
		os.Exit(1)
	}
...
func child() {
	if len(os.Args) < 3 {
		fmt.Fprintln(os.Stderr, "child: no command given")
		os.Exit(1)
	}
